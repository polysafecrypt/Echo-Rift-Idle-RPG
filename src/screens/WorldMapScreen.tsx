...
import { COLORS, CLASS_INFO, QUEST_CONFIGS, PRESTIGE } from '../constants'
        ...
        <TouchableOpacity style={styles.profileBtn} onPress={() => navigation.navigate('Profile')} activeOpacity={0.8}>
          ...
          <View>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
              ...
              {player.level >= PRESTIGE.threshold(player.prestige_tier ?? 0) && (
                <Text style={styles.rebirthReadyDot}>🌌</Text>
              )}
            </View>
            <Text style={[styles.profileClass, { color: classInfo?.color || COLORS.neonGreen }]}>
              LVL {player.level} {classInfo?.name?.toUpperCase()}
              {player.level >= PRESTIGE.threshold(player.prestige_tier ?? 0) ? ' · REBIRTH READY' : ''}
            </Text>
          </View>
        </TouchableOpacity>
